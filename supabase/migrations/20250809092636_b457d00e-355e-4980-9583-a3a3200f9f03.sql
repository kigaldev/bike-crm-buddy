-- 1) Tabla de incidencias interna por empresa
CREATE TABLE IF NOT EXISTS public.incidencias_saas (
  ...
  detalle text,
  severidad text NOT NULL DEFAULT 'alta', -- valores: critica|alta|media|baja
  estado text NOT NULL DEFAULT 'Abierta', -- valores: Abierta|En curso|Resuelta
...
CREATE POLICY IF NOT EXISTS incidencias_read_miembros
...
-- Inserción/Actualización/Eliminación: solo admin o qa activos
CREATE POLICY IF NOT EXISTS incidencias_write_admin_qa
ON public.incidencias_saas
FOR ALL
USING (
  EXISTS (
...
-- 3) Función para crear incidencia (SECURITY DEFINER) + notificaciones automáticas
...
BEGIN
  ...
  -- Notificar a admins si severidad es alta o critica
...
-- 4) RPCs auxiliares (opcional): cambiar estado rápidamente
